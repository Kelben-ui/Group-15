...
const cardVariants = cva(
  'rounded-lg border border-border bg-card text-card-foreground shadow-sm transition-all duration-200',
  {
    variants: {
      hover: {
        true: 'hover:shadow-md hover:border-primary/50 cursor-pointer',
        false: '',
      },
      variant: {
        default: '',
        ...
        success: 'border-l-4 border-l-secondary bg-secondary/5',
...
export const Card = React.forwardRef<HTMLDivElement, CardProps>(
  ({ className, hover, variant, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(cardVariants({ hover, variant }), className)}
      {...props}
    />
  )
);
